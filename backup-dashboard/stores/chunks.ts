...
export const useChunksStore = defineStore("chunks", () => {
  ...
  const isHourIndeterminate = (hour: number) =>
    computed(() => {
      const group = groups.value.find((g) => g.hour === hour);
      if (!group || group.chunks.length === 0) return false;
      const selectedInHour = group.chunks.filter((c) =>
  ...
  const colorScale = [
    "#1a4d1c", // 0 - visible dark green (was near-black)
    "#236b27",
    "#2d9134",
    "#39b843",
    "#52cc5c",
  ...
  async function fetchChunks() {
    ...
    try {
      ...
      error.value =
        e instanceof Error ? e.message : "Failed to load chunk data";
    } finally {
  ...
  function removeChunks(ids: string[]) {
    ...
    groups.value = groups.value
      .map((g) => ({
        ...
        stats: {
          ...
          totalRecords: g.chunks
            .filter((c) => !idSet.has(c.id))
            .reduce((s, c) => s + c.dataCount, 0),
          totalSizeBytes: g.chunks
            .filter((c) => !idSet.has(c.id))
            .reduce((s, c) => s + c.sizeBytes, 0),
          totalRawSizeBytes: g.chunks
            .filter((c) => !idSet.has(c.id))
            .reduce((s, c) => s + c.rawSizeBytes, 0),
        },
      }))
    ...
    if (meta.value) {
      ...
      meta.value = {
        ...
        minDataCount: remaining.length
          ? Math.min(...remaining.map((c) => c.dataCount))
          : 0,
        maxDataCount: remaining.length
          ? Math.max(...remaining.map((c) => c.dataCount))
          : 0,
      };
    }
  }
